...
std::shared_ptr<Expr> Parser::ParseTermExpr()
{
  ...
  switch (tk.GetKind()) {
    ...
    case Token::Kind::INT: {
      std::uint64_t integer(tk.GetInteger());
      lexer_.Next();
      return std::static_pointer_cast<Expr>(
...
std::shared_ptr<Expr> Parser::ParseAddSubExpr()
{
  ...
  while (currentKind == Token::Kind::PLUS || currentKind == Token::Kind::MINUS) {

    lexer_.Next();
    auto rhs = ParseCallExpr();
    if(currentKind == Token::Kind::PLUS){
      // lexer_.Next();
     term = std::make_shared<BinaryExpr>(BinaryExpr::Kind::ADD, term, rhs);
    } 
    if(currentKind == Token::Kind::MINUS){ 
      //  lexer_.Next();
