...
// -----------------------------------------------------------------------------
Token Token::Integer(const Location &l, const std::uint64_t str)
{
  Token tk(l, Kind::INT);
...
const Token &Lexer::Next()
{
  ...
  switch (chr_) {
    ...
    default: {
      ...
      if(isdigit(chr_)){
        std::uint64_t auxNumber = 0;
        while(isdigit(chr_)){
          auxNumber = auxNumber*10 + chr_ - '0';
          NextChar(); 
        }
       return tk_ = Token::Integer(loc,auxNumber);
      }
